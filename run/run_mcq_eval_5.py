...
MODELS = [
    ...
    # "ilsp/Meltemi-7B-Instruct-v1.5", # wrong
    #arabic models not done
    # "inceptionai/jais-13b", # wrong
    # "inceptionai/jais-13b-chat", # wrong
    # "inceptionai/Jais-2-8B-Chat",
    # "FreedomIntelligence/AceGPT-v2-8B", # wrong
...
# Localized prompt templates: {lang: (question_label, instruction, answer_label, idk_option)}
PROMPT_LANG = {
    "zh": (
        "问题：",
        "仅用字母（A、B、C、D 或 E）作答。",
    ...
    "el": (
        "Ερώτηση: ",
        "Απαντήστε μόνο με το γράμμα (A, B, C, D ή E).",
        "Απάντηση:",
        "Δεν ξέρω",
    ),
    "ar": (
        "السؤال: ",
        "أجب بالحرف فقط (A أو B أو C أو D أو E).",
        "الجواب:",
        "لا أعرف",
    ),
    "hi": (
        "प्रश्न: ",
        "केवल अक्षर (A, B, C, D, या E) से उत्तर दें।",
        "उत्तर:",
        "मुझे नहीं पता",
    ),
    "id": (
        "Pertanyaan: ",
        "Jawab hanya dengan huruf (A, B, C, D, atau E).",
        "Jawaban:",
        "Saya tidak tahu",
    ),
    "ko": (
        "질문: ",
        "알파벳(A, B, C, D 또는 E)으로만 답하세요.",
        "답변:",
        "모르겠습니다",
    ),
    "en": (
...
def build_prompt(row: dict, lang: str = "en", tok=None, instruct: bool = False, model_id: str = "") -> str:
    q_label, instruction, ans_label, idk_option = PROMPT_LANG[lang]
    text = (
...
def extract_answer(text: str) -> str:
    """Return the first A/B/C/D/E found in the generated text, or '' if none."""
    text = text.strip()
...
def evaluate_model(model_id: str, subsets: list[str], batch_size: int, max_new_tokens: int, max_samples_per_subset: int | None = None, local_only: bool = False) -> tuple[dict, list]:
    ...
    for subset in subsets:
        ...
        abstained = sum(p == "E" for p in preds)                   # chose "I don't know"
        conf_wrong = sum(p != g and p != "E" for p, g in zip(preds, gold))  # wrong and not abstained

        # Accuracy: fraction of correct answers out of all questions
        acc = correct / n if n else 0.0

        # AbstainRate: fraction of questions where the model chose E ("I don't know")
        abstain_rate = abstained / n if n else 0.0

        # ConfErrRate (Confident Error Rate): fraction of questions where the model
        # committed to a wrong answer (did NOT abstain with E)
        conf_err_rate = conf_wrong / n if n else 0.0

        # CondAcc (Conditional Accuracy): accuracy among questions the model
        # attempted (i.e., excluding abstentions) — how reliable the model is
        # when it does commit to an answer
        attempted = correct + conf_wrong   # questions answered with A-D
...
def main():
    ...
    ap.add_argument("--outdir", default="results/lite_eval_v1_5")
    ...
    for model_id in args.models:
        accuracy, abstain_rates, conf_err_rates, cond_accs, raw_records = evaluate_model(
            model_id, args.subsets, args.batch_size, args.max_new_tokens, args.max_samples_per_subset, local_only=local_only
        )
        slug = model_slug(model_id)

        # Per-model accuracy JSON — each metric is a flat {subset: value} dict,
        # matching the structure of the original "accuracy" field.
        acc_path = outdir / f"{slug}_accuracy.json"
        with open(acc_path, "w", encoding="utf-8") as f:
            json.dump({
                "model":         model_id,
                "accuracy":      accuracy,       # acc = correct / total
                "abstain_rate":  abstain_rates,  # fraction of E ("I don't know") choices
                "conf_err_rate": conf_err_rates, # fraction of wrong non-abstain answers
                "cond_acc":      cond_accs,      # acc among attempted (non-abstain) questions
